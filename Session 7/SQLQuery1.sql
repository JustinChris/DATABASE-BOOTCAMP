...
-- 6
SELECT 
	CONVERT(VARCHAR, hss.TransactionDate, 106) as [Date],
...
-- 8
SELECT 
	LEFT(s.StaffName, CHARINDEX(' ', s.staffName) - 1) as [StaffName],
	hss.TransactionId,
	COUNT(*) as [Total Treatment per Transaction]
FROM MsStaff as s
...
JOIN DetailSalonServices as dss on hss.TransactionId = dss.TransactionId
JOIN MsTreatment as t on dss.TreatmentId = t.TreatmentId
WHERE DATENAME(WEEKDAY, hss.TransactionDate) like 'Thursday' and s.StaffName like '%Ryan%'
